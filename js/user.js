...
async function renderUserInfo() {
  ...
  try {
    ...
    infoDiv.innerHTML = `
      ...
      <p><strong>Ngày tạo:</strong> ${formatDate(userData.createdAt)}</p>
    `;
  } catch (error) {
...
async function renderUserOrders() {
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/orders/account/${userId}`, { 
      headers: {
        Authorization: `Bearer ${getToken()}`,
      },
    });
    ...
    if (ordersData && ordersData.length > 0) {
      ordersData.forEach(order => {
        ...
        const productNames = order.orderItemDetails.map(item => item.name).join(', '); 
        row.innerHTML = `
          <td>${order.id}</td>
          <td>${productNames}</td>
